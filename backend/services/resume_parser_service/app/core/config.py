"""Configuration settings for Resume Parser Service."""
...
class ResumeParserConfig(BaseConfig):
    """Resume Parser Service configuration extending BaseConfig."""
    ...
    # ═══════════════════════════════════════════════════════════
    # ⚡ NEW: Redis Configuration (ADD THESE LINES)
    # ═══════════════════════════════════════════════════════════
    ...
    # AI/LLM Configuration (existing - keep as is)
    ...
    # Weights & Biases (existing - keep as is)
    ...
    # Tesseract (existing - keep as is)
    tesseract_cmd: str = Field(
        default="/usr/local/bin/tesseract", alias="TESSERACT_CMD"
    )

    # Logging (existing - keep as is)
    log_level: str = Field(default="INFO", alias="LOG_LEVEL")
    log_format: str = Field(default="json", alias="LOG_FORMAT")

    # Properties (existing - keep as is)
    @property
    def allowed_extensions_list(self) -> list[str]:
        """Split comma-separated extensions into list."""
        return [ext.strip() for ext in self.allowed_extensions.split(",")]

    @property
    def llm_providers_list(self) -> list[str]:
        """Get list of LLM providers in priority order."""
        return [p.strip() for p in self.llm_provider_priority.split(",")]

    @property
    def service_port(self) -> int:
        """Get resume parser service port."""
        return self.resume_parser_service_port

    @property
    def database_url(self) -> str:
        """Override to use resume parser database."""
